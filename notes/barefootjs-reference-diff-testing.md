...
3. （許されない）クリーンにコンパイルできるのに、壊れた bundle を出す——**サイレントホール**

3番目が「テストで検出すべきバグ」の定義そのもの。`KNOWN_HOLES` という**縮小専用**のリストに現在のサイ
レントホールを列挙し、直ったエントリをテストが消し忘れなく検出する（[[pairwise-testing]] の
...
---
コンパイルはクリーンに通るのに、壊れた client bundle を出す状態（サイレントホール）。コンパイルが
