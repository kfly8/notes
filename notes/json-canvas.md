...
- **線の経路**: [[barefootjs]] の `@barefootjs/xyflow` に任せる。JSON Canvas のノードを xyflow の内部ノードに変換し、線が付く位置にハンドルを置いて、`computeEdgePosition` と `getEdgePath`（`step` 型。中身は `@xyflow/system` の `getSmoothStepPath` で角を丸めないもの）で経路を引く。出る辺と入る辺の向きが違えば一度だけ折れ（L 字）、向きが同じなら中間で二度折れ、同じ辺どうし（下から下など）は 20px 外へ張り出してコの字に回る。
- **ラベルの位置**: 相手に入っていく側の線に載せる。二度折れる線とコの字では中央の線に載せる。xyflow もラベルの位置を返すが、L 字では出ていく側の線の中点になるので使わず、返ってきたパスの角から自分で決めている。
