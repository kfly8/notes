...
構成図・フロー・シーケンスは、ASCII アートより図を優先する。図は ` ```canvas ` のフェンスに
[JSON Canvas](https://jsoncanvas.org/spec/1.0/) を書き、ビルド時に `src/plugins/canvas.ts` が SVG にする。
座標と線の出入りする辺（`fromSide` / `toSide`）を自分で決めるので、配置を自動で決める記法と違って、見た目を
細かく整えられる。mermaid は使わない（配置と配線を自動で決めるため、箱の幅や線の折れ方を制御できない）。

- 1枚の図には1つの流れだけを描く。詰め込みたくなったら図を分けるか、表に任せる。
- 流れは上から下に並べる。横に広げるとスマホで文字が小さくなる。
- 箱の幅・高さと間隔は、図の中でそろえる。ラベルを載せる線には 80px ほどの間隔を取る。
- 色は強調したい箱や線に `"color": "4"` だけを使い、何を強調しているかを図の直後の本文で説明する。
  サイトのアクセント色で塗られ、テーマに追従する。16進数の色は無視される。
- 戻りの流れや補助的な関係は、仕様外の拡張 `"style": "dashed"` で点線にする。
- グループ（`"type": "group"`）は、箱をまとめる枠や、シーケンスのレーンに使う。
- 線の引き方:
  - 辺を省略すると、相手の方向の辺から出入りする。
  - 出る辺と入る辺の向きが違えば一度だけ折れ（L 字）、同じ向きなら中間で二度折れる。
  - 同じ辺どうし（下から下など）は、外へ張り出してコの字に回る。
  - ラベルは、相手に入っていく側の線に載る（二度折れる線とコの字では中央の線）。
  - 1つの辺に矢印の先が集まるときは、付く位置が自動でずれる。
- 例は `notes/tagpr-workers-builds-release-flow.md`（分岐）、`notes/mcp-v2.md`（グループと往復）、
  `notes/tauri-sync-command-blocks-repaint.md`（シーケンス）。
